...
m1 = MassFunction({'S':0.4,'L':0.1,'R':0.3, 'SL':0.05,'SLR':0.15})
m2 = MassFunction({'S':0.3,'L':0.1,'R':0.25,'SLR':0.35,})
hypotheses =  [857.85931002,  886.28508522,  914.71086043,  943.13663563,
        971.56241084,  999.98818604, 1028.41396125, 1056.83973645,
       1085.26551166, 1113.69128686, 1142.11706207]
masses =[9.14662830e-05, 4.88992821e-04, 2.16704732e-03, 5.69201715e-03,
       9.28030979e-03, 9.14311037e-03, 5.67090955e-03, 2.12131418e-03,
       4.43259679e-04, 8.09124811e-05]

F_vec_gauss = [(frozenset({hypothesis}),mass) for hypothesis, mass in zip(hypotheses,masses)]
m3 = MassFunction(F_vec_gauss)
rand_nums = np.zeros((100,))
samples = m3.sample(100, quantization=False)
for i,sample in enumerate(samples):
    for e in sample: break
    rand_nums[i]=e
    
plot.hist(rand_nums, bins=hypotheses)
plot.show()
